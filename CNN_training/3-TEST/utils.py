...
##    img_label = sm.label(img_open,connectivity=1)
    #img = filters.threshold_adaptive(img, 15, 'gaussian')
    img=filters.gaussian(img, sigma=0.85)
    img=equalize_hist(img)
    #imwrite('processed.jpg', img*255)
    img=(img > 0.7)*1.0
    #imwrite('processed.jpg', img*255)
...
##            cnt += 1
    ...
    for i in range(np.max(img)):
        if props[i]['area'] > 290:
            ...
            cnt += 1
            #print(np.min(np.array(width)))
    #imwrite('processed.jpg', image_region*255)
