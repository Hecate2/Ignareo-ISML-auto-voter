...
def selector(html,voting_token,vcode):
    select=[#设置二选一投票。写在前面的优先被满足
        #同时用于多人选多人的arena
        ["珂朵莉",2.0],#角色，投这个角色的概率(大于等于1就是必投)；设置整数-1表示放空这个角色所在的组，完全不投。放空也是有优先级的！
        ["克梅修",2.0],
        ["奈芙莲",2.0],
    ]
    rank=[#设置给数字的投票
        ...
        ["珂朵莉",1,0.4,3],#角色名，愿意给的最好数字，概率，愿意给的最差数字
        ["奈芙莲",1,0.4,3],
        ["克梅修",1,0.4,3],
        #[n,角色名，角色名，更多角色名……]
    ...
    selectDensity=random.random()*0.2+(0.35 if random.random()<0.4 else 0.75)  #二选一的选择密度，同时用于多人选多人的比赛（不含外卡赛）
    #selectDensity=0.561
    selectRate=random.random()*0.2+0.524 #给一个新数字的概率，同时用于给分数的比赛
    #selectRate=0.764
    wildcardDensity=random.random()*0.2+0.152 #多人选多人的外卡赛选择密度(越低越有利于要选的角色)
